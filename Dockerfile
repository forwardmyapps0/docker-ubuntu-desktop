...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    gnome-session gnome-terminal gnome-settings-daemon epiphany-browser metacity nautilus \
    novnc websockify python3 openssl dbus-x11 xauth
...
CMD bash -c " \
    ...
    -xstartup 'gnome-session --session=gnome-flashback-metacity' && \
